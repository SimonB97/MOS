...
@click.option(
    "--output", "-o", type=click.Path(), help="Output directory (default: ./mp3)"
)
@click.option(
    "--workers",
    "-w",
...
def convert(input_path: str, output: str | None, workers: int) -> None:
    ...
    if src.is_file():
        ...
        files = [
            f for f in src.rglob("*.*") if f.suffix.lower() in {".flac", ".wav", ".m4a"}
        ]
    ...
    for f in files:
        if dst:  # Output directory was specified
            if src.is_dir():
                rel = f.relative_to(src)
                out = dst / rel.with_suffix(".mp3")
            else:  # src is file
                out = dst / f.with_suffix(".mp3").name
        else:  # No output directory specified, save alongside input
            if src.is_dir():
                # Save in the same relative structure within the input directory
