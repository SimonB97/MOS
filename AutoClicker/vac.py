...
class CalibrationManager:
    ...
    def _select_button_match(self, matches: List[ButtonMatch]) -> Optional[ButtonMatch]:
        ...
        if len(matches) == 1:
            ...
            self._show_candidate(match, "Single candidate found")
            return match
            
        print("\nMultiple button candidates found. Please select the correct one:")
        
        # Create a window to show all candidates side by side
        preview_images = []
        for i, match in enumerate(matches):
            ...
            labeled_img = cv2.copyMakeBorder(
                button_img, 
                0, 30, 0, 0, 
                cv2.BORDER_CONSTANT, 
                value=[255, 255, 255]
            ...
            cv2.putText(
                labeled_img,
                f"Candidate {i + 1} (conf: {match.confidence:.2f})",
                (5, button_img.shape[0] + 20),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.5,
                (0, 0, 0),
                1
            )
            preview_images.append(labeled_img)
        
        # Combine all images horizontally
        preview = np.hstack(preview_images)
        cv2.imshow("Button Candidates", preview)
        cv2.waitKey(1)  # Show window without blocking
        
        # Highlight each region on screen
        for i, match in enumerate(matches):
